...
import { Github, Chrome, LogOut, Loader2, X, CloudOff } from 'lucide-react'
import { useLangStore } from '../../store/langStore'

export function AuthModal() {
  const { authModalOpen, setAuthModalOpen, session, user, profile, isSyncing, signInWithGitHub, signInWithGoogle, signOut, fetchProfile } = useAuthStore()
  const { t } = useLangStore()

  // Restore session from Supabase on mount (handles token refresh)
  useEffect(() => {
    if (!supabase) return
...
function LoginView({ onGitHub, onGoogle, t }: {
  onGitHub: () => void
  onGoogle: () => void
  t: (k: string) => string
}) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      {!hasSupabase ? (
        ...
        <>
          <OAuthBtn onClick={onGitHub} icon={<Github size={15} />} label="GitHub" />
          <OAuthBtn onClick={onGoogle} icon={<Chrome size={15} />} label="Google" />
        </>
      )}
    </div>
  )
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          {isSyncing ? <Loader2 size={12} style={{ color: 'var(--color-accent)', animation: 'spin 1s linear infinite' }} /> : (
      ...
      <button
        ...
        style={{
          ...
          color: 'var(--color-text-dim)', fontSize: '12px', cursor: 'pointer',
          transition: 'all 0.1s',
        }}
